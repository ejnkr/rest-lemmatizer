use crate::store::{StoreImpl, Store};
...
impl Score {
    pub fn new(noun_probability: f32, count: u32, unique_postfixes_count: u32) -> Self {
        Self {
            noun_probability, count, unique_postfixes_count
        }
    }
}
...
impl State {
    ...
    fn postfix_noun_prob(
        ...
        let with_lastchar = match count {
            Some(count) if count.postnoun + count.postother >= 100 => {
                ((count.postother as f64 + self.smooth_factor) / ((count.postnoun + count.postother) as f64 + self.smooth_factor)).ln() - 
                ((count.postnoun as f64 + self.smooth_factor) / ((count.postnoun + count.postother) as f64 + self.smooth_factor)).ln()
            }
            _ => 0.0,
        };
    ...
    pub fn extract_nouns2(&self, text: &str) -> Result<Vec<(String, Score)>> {
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            for i in 0..chars.len() {
                ...
                for j in 1..MAX_POSTFIX_SIZE.min(chars.len() - i - 1) {
                    ...
                    let word = (chars[word_start_index..(i + 1 + j)].iter().cloned().collect::<Vec<char>>(), j);
        ...
        for ((word, suffix_len), count) in words.into_iter() {
            let candidate = word[..word.len()-suffix_len].into_iter().collect();
            let suffix = word[word.len()-suffix_len..].iter().collect::<String>();
            let prob = self.postfix_noun_prob(word[word.len()-suffix_len-1], suffix.clone(), noun_count, other_count)?;
        ...
        res.sort_by(|(_, s1), (_, s2)| {
            if s1.noun_probability.is_nan() && s2.noun_probability.is_nan() {
                ...
                ((s2.count as f32).log10() * s2.noun_probability * s2.unique_postfixes_count as f32)
                    .partial_cmp(&((s1.count as f32).log10() * s1.noun_probability * s1.unique_postfixes_count as f32))
                    .unwrap()
            }
        });
        Ok(res)
    ...
    pub fn extract_nouns(&self, text: &str) -> Result<Vec<(String, Score)>> {
            ...
            for i in 0..chars.len() {
                ...
                for j in 1..MAX_POSTFIX_SIZE.min(chars.len() - i - 1) {
                    ...
                    if j == 1 && (nl != 1.0 && ol != 1.0) {
                        unique_postfixes.insert(chars[word_start_index..(i + 2).min(chars.len())].iter().collect::<String>());
                    }
        ...
        unique_postfixes.into_iter().for_each(|up| {
            ...
            let Score { unique_postfixes_count, .. } = likelihoods.entry(chars[..chars.len()-1].into_iter().collect::<String>()).or_insert(Score::default());
